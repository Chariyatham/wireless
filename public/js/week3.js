...
// v2: + walkthrough โจทย์เดินสด + runner + TimedExam (เครื่องมือกลางจาก stepper.js)
import { mountWalk, mountRunner, mountExam } from './stepper.js';
...
// ---------------------------------------------------------------
// Walkthrough: โจทย์เดินสดของอาจารย์ (B = 1 MHz, SNR 24 dB → 8 Mbps → M = 16)
// ตัวเลขทุกตัวยืนยันด้วยโปรแกรมแล้ว (10^2.4 = 251.19, log2(252) = 7.977)
// ---------------------------------------------------------------
mountWalk('walk-shannon', [
  { title: 'ขั้น 1 · หาแบนด์วิดท์ B จากสเปกตรัม',
    body: `สเปกตรัม 3 MHz ถึง 4 MHz
B = ขอบบน − ขอบล่าง = 4 − 3 = 1 MHz = 10⁶ Hz`,
    note: 'กับดักแรกของทรงนี้: แบนด์วิดท์คือ "ความกว้าง" ของช่วง ไม่ใช่เลขขอบบน — ใครหยิบ 4 MHz ไปเข้าสูตรคือพลาดตั้งแต่ขั้นแรก' },
  { title: 'ขั้น 2 · ถอด SNR จาก dB (ทักษะ Week 1 กลับมา)',
    body: `SNR_dB = 24
24 = 10·log₁₀(SNR)  →  log₁₀(SNR) = 2.4
SNR = 10^2.4 ≈ 251.2 ≈ 251`,
    note: 'อาจารย์เดินขั้นนี้สด ๆ กลางห้อง: "หารสิบ แล้วยกกำลังถอด log" — สูตร Shannon ต้องใช้ SNR ที่เป็นอัตราส่วน ห้ามเอาค่า dB ยัดเข้าสูตรตรง ๆ เด็ดขาด' },
  { title: 'ขั้น 3 · เข้าสูตร Shannon + เปลี่ยนฐาน log',
    body: `C = B·log₂(1 + SNR) = 10⁶ × log₂(252)
เครื่องคิดเลขไม่มีปุ่ม log₂ → เปลี่ยนฐาน (Week 1 อีกรอบ!):
log₂(252) = log₁₀(252) ÷ log₁₀(2) = 2.401 ÷ 0.301 ≈ 7.98 ≈ 8
C ≈ 8 × 10⁶ = 8 Mbps`,
    note: '🎙 อาจารย์หยุดถามทั้งห้องตรงนี้: "เครื่องคิดเลขมี log₂ ไหม? ไม่มี! แล้วทำไง?" — สมบัติเปลี่ยนฐานจาก Week 1 โผล่ในข้อสอบผ่านโจทย์แบบนี้ ไม่ได้มาถามตรง ๆ · ทริคเช็กเร็ว: 252 ≈ 256 = 2⁸ → log₂ ≈ 8' },
  { title: 'ขั้น 4 · Nyquist ย้อนหา M (ต้องใช้กี่ระดับ)',
    body: `Nyquist: C = 2B·log₂M
8×10⁶ = 2(10⁶)·log₂M
log₂M = 8×10⁶ ÷ 2×10⁶ = 4
M = 2⁴ = 16 ระดับ`,
    note: 'สองสูตรทำงานเป็นคู่: Shannon บอก "เพดานที่ noise ยอมให้" แล้ว Nyquist ย้อนบอกว่า "ต้องออกแบบสัญญาณกี่ระดับถึงจะไปแตะเพดานนั้น"' },
  { title: 'สรุป + วิธีเช็กคำตอบก่อนส่ง',
    body: `คำตอบ: C ≈ 8 Mbps ด้วยสัญญาณ M = 16 ระดับ
เช็กย้อน: 2 × 1MHz × log₂(16) = 2×10⁶×4 = 8 Mbps ✓ ตรงกัน
เช็กสามัญสำนึก: 16 ระดับ = 4 บิต/สัญลักษณ์ × 2B สัญลักษณ์/วิ ✓`,
    note: 'ทรงนี้คือทรงโปรดของข้อสอบ: ให้สเปกตรัม + SNR_dB แล้วถามทั้ง C และ M — ฝึกให้ไหลทั้ง 4 ขั้นโดยไม่ต้องเปิดชีท (ลองอีกรอบกับข้อสอบจำลองข้อ 2 ข้างล่าง)' },
]);

// ---------------------------------------------------------------
// JS runner: โจทย์ครูทั้งข้อ + ฟังก์ชันใช้ซ้ำ
// ---------------------------------------------------------------
mountRunner('runner', `// โจทย์เดินสดของอาจารย์ — แก้สองบรรทัดบนเป็นโจทย์อื่นได้เลย
const B = 1e6;        // แบนด์วิดท์ (Hz) — จากสเปกตรัม 3–4 MHz
const SNRdB = 24;     // SNR ในหน่วย dB

// ขั้น 2: ถอด dB → อัตราส่วน
const SNR = 10 ** (SNRdB / 10);
console.log("SNR =", SNR.toFixed(1), "เท่า");

// ขั้น 3: Shannon (log2 = เปลี่ยนฐานจาก log10)
const log2 = (x) => Math.log10(x) / Math.log10(2);
const C = B * log2(1 + SNR);
console.log("C =", (C / 1e6).toFixed(2), "Mbps  (เพดานตามทฤษฎี)");

// ขั้น 4: Nyquist ย้อนหา M
const M = 2 ** (C / (2 * B));
console.log("ต้องใช้ M =", M.toFixed(1), "ระดับ  (ปัดเป็นกำลังของ 2 ที่ใกล้สุด)");

// โบนัส: ตาราง SNR_dB → C เพื่อเห็นว่า "อัดกำลังส่ง" ให้ผลหน่วงลงเรื่อยๆ
console.log("\\nSNR_dB → C (B = 1 MHz):");
for (const db of [6, 12, 18, 24, 30, 36])
  console.log(" ", db, "dB →", (B * log2(1 + 10 ** (db / 10)) / 1e6).toFixed(2), "Mbps");`);

// ---------------------------------------------------------------
// TimedExam — ข้อสอบจำลอง week 3
// ---------------------------------------------------------------
mountExam([12, 18, 25]);
