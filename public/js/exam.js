// ===== คลังข้อสอบรวม 4 สัปดาห์ + สอบจำลองจับเวลา (/exam) =====
// โจทย์ชุดเดียวกับข้อสอบจำลองท้ายบท w1–w4 — ตัวเลขยืนยันด้วยโปรแกรมแล้วทุกข้อ
...
// ---- ชุดสอบ ----
const MODES = [
  { id: 'full', name: '📚 ชุดเต็ม 21 ข้อ (ซ้อมใหญ่)', pick: () => [...BANK] },
  { id: 'mix12', name: '🎲 สุ่ม 12 ข้อ (3 ต่อสัปดาห์)', pick: () => pickBalanced(3) },
  { id: 'mix8', name: '⚡ สุ่ม 8 ข้อ (2 ต่อสัปดาห์)', pick: () => pickBalanced(2) },
];
...
function pickBalanced(perWeek) {
  const out = [];
  for (const w of [1, 2, 3, 4]) out.push(...shuffle(BANK.filter((q) => q.w === w)).slice(0, perWeek));
  return out;
}
